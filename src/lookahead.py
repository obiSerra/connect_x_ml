...
def multistep_agent_factory(move_predict=1):
    def multistep_agent(observation, configuration):
        ...
        possibilities = look_ahead(board_2d, configuration, step_predict)
        bests = select_bests(possibilities)
    #    if obs.board[int(col)] == 0
        valid_moves = [b[0]
                       for b in bests if observation.board[int(b[0])] == 0]
